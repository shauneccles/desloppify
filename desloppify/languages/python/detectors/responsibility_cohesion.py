...
def detect_responsibility_cohesion(
    ...
    *,
    ...
    batch_results = process_files_parallel(
        files=files,
        worker_func=_responsibility_batch_worker,
        mode="extend",
        min_files=100,
        task_name="python responsibility cohesion",
        min_loc=min_loc,
...
def _process_responsibility_file(
    filepath: str,
    min_loc: int,
    min_functions: int,
) -> dict:
...
def _responsibility_batch_worker(
    files: list[str],
    min_loc: int,
    min_functions: int,
) -> dict:
    ...
    for filepath in files:
        result = _process_responsibility_file(filepath, min_loc, min_functions)
        entry = result.get("entry")
        if isinstance(entry, dict):
            entries.append(entry)
        candidates += int(result.get("candidate", 0))
